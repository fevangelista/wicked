...
def gen_op(label, rank, cre_spaces, ann_spaces, diagonal=True, only_terms=False):
    ...
    if isinstance(rank, tuple) or isinstance(rank, list):
        ...
        if (cre_rank != ann_rank) and (not diagonal):
            raise ValueError(
                "diagonal=False is not well defined for rank[0] != rank[1]"
            )
        ...
        for le in itertools.product(*cre):
            ...
            if is_le_sorted:
                for re in itertools.product(*ann):
                    is_re_sorted = all(
                        m[re[i]] >= m[re[i + 1]] for i in range(len(re) - 1)
                    )
                    if is_re_sorted:
                        if not diagonal:
                            if le != re:
...
def dict_to_einsum(eq_dict, optimize="'optimal'", no_np=False):
    lhs = eq_dict["lhs"][0][0]
    if eq_dict["lhs"][0][1] != "":
        lhs += "['" + eq_dict["lhs"][0][1] + "']"
    rhs = "np.einsum('" if not no_np else "einsum('"
    indices = ""
...
def compile_einsum(
    equation, keys=None, return_eq_dict=False, optimize="'optimal'", no_np=False
):
    """
        ...
        for i in tensor.upper() + tensor.lower():
            ...
            if index_dict.get(i):
                ...
                try:
                    ...
                    raise ValueError(
                        f"Index {i} is not available for einsum contraction. Try enlarging the index pool."
                    )
                index_dict[i] = index
                indstr += index
    ...
    for t in rhs.tensors():
        tensor_label = "".join([str(_)[0] for _ in t.upper()]) + "".join(
...
def equation_to_dict(eq):
    ...
    res["lhs"] = [
        lhs.label(),
        [str(_) for _ in lhs.lower()],
        [str(_) for _ in lhs.upper()],
    ]
    ...
    for i in rhs.tensors():
        res["rhs"].append(
            [i.label(), [str(_) for _ in i.lower()], [str(_) for _ in i.upper()]]
        )

...
def test_expression3():
    """Test the normal_ordered method of the Expression class on commutators of operators"""
    w.reset_space()
    w.add_space("g", "fermion", "unoccupied", list("abcdefghijklmnopqrstuvwxyz"))
    ...
    c1 = (o @ k) - (k @ o)
    c1 = c1.normal_ordered(True)
    test_c1 = w.expression("t^{}_{} a+(g_3) a-(g_2)")
    assert c1 == test_c1

    c2 = (c1 @ k) - (k @ c1)
    c2 = c2.normal_ordered(True)
